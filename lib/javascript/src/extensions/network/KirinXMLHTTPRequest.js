...
function XMLHTTPRequest (params) {
    if (!instance || !instance.nativeObject) {
        throw new Error("The KirinXMLHTTPRequest extension hasn't loaded properly, or is shutting down");
    }
}
...
XHR._doOnInterrupt = function (networkConnectedness, errorMessage) {
    var self = this;
    if (networkConnectedness) {
        // server side timeout
        console.error(errorMessage);
    } else {
        // TODO the network has dropped. We should probably do something about that.
