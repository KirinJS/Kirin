...
var Native = require("./Native"),
    ...
    native2js = {},
    callbacks = {}, nativeAccessibleJavascriptObjects = {},
    loadedObjects = {}, 
 ...
 */
native2js.execMethod = function (moduleName, methodName, argsList, futureId) {
    ...
    try {
        if (argsList) {
            retValue = module[methodName].apply(module, argsList);
        } else {
            retValue = module[methodName]();
        }
    } catch (e) {
    ...
    if (futureId) {
        return Native.handleReturnValue(futureId, retValue);
    }
    
};

/**
 * Call a previously "wrapped" callback, from native.
 *
 * This finds and executes a callback function which was stored in the process of calling native 
 * from a extension. Each extension had to manually replace each function that was going called 
 * from native with a token (in <code>wrapCallback</code> and <code>wrapCallbacks</code>.
 * 
 * This was problematic for a number of reasons: 
 * <ul>
 * <li>It was manual and everyone rolled their own.</li>
 * <li>The native object had to keep track of callbacks to be deleted. Yuck.</li>
 * <li>The context <code>this</code> was lost for each callback</li>
 * </ul>
 * 
 * This is now Deprecated, and will not be supported in future releases. The preferred way of doing 
 * callbacks is to use IDL. 
 * No more extensions will be written with this mechanisms, and once all extensions are modified to use IDL, 
 * this way of doing callbacks will be removed.
 * @Deprecated
 */
...
/**
 * Execute a method on an object that has been passed to native.
 * 
 * The object with the given id is resolved. If the method exists, then it is called.
 * 
 * If the method returns <code>falsey</code>, then the object will be disposed of and not available 
 * for further callbacks from native. This is to reduce the complexity of native extensions.
 */
native2js.execCallbackMethod = function (objectId, methodName, argsList) {
    var object = nativeAccessibleJavascriptObjects[objectId],
        callback,
        maintainObject = false;
    if (!object) {
        console.warn("Callback object " + objectId + " doesn't exist. Couldn't call " + methodName);
        return;
    }
    callback = object[methodName];
    if (typeof callback !== 'function') {
        console.error("Callback method " + methodName + " doesn't exist on object " + objectId);
        
        // unsure what to do w.r.t. object disposal.
        maintainObject = false;
    } else {
        try {
            maintainObject = callback.apply(object, argsList);        
        } catch (e) {
            handleError("calling callback " + objectId + methodName + JSON.stringify(argsList), e);
        }
    }
    
    if (!maintainObject) {
        delete nativeAccessibleJavascriptObjects[objectId];
    }    
};

/**
 * This method will be rarely used, as the callback methods look after object disposal.
 * 
 * However, in the case of persistent listeners, or optional methods, this may be used.
 */
native2js.deleteCallbackObject = function (args) {
    var i=0, max;
    for (max=args.length; i<max; i++) {
        delete nativeAccessibleJavascriptObjects[args[i]];
    }
